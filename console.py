...
class HBNBCommand(cmd.Cmd):
    ...
    def do_all(self, arg):
        ...
        argl = parse(arg)
        if argl and argl[0] not in self.__classes:
            print("** class doesn't exist **")
        else:
            obj_list = []
    ...
    def do_count(self, arg):
        ...
        argl = parse(arg)
        count = sum(1 for obj in storage.all().values() if argl and argl[0] == obj.__class__.__name__)
        print(count)
    ...
    def do_update(self, arg):
       ...
       <class>.update(<id>, <dictionary>)
        ...
        if len(argl) < 2:
            print("** instance id missing **")
            return False
        if f"{argl[0]}.{argl[1]}" not in objdict:
            print("** no instance found **")
            return False
        if len(argl) < 3:
